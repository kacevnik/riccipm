...
if( get_adjacent_post(false, '', true) ) { 
  previous_post_link('%link', '<i class="fas fa-long-arrow-alt-left"></i> Предыдущий проект' ); 
}
else { 
	$first = new WP_Query('posts_per_page=1&order=DESC');
	$first->the_post();

	echo '<a class="go_to_prev_project" href="' . get_permalink() . '">← Предыдущий проект</a>';

	wp_reset_postdata();
}; 

if( get_adjacent_post(false, '', false) ) { 
	next_post_link('%link', 'Следующий проект <i class="fas fa-long-arrow-alt-right"></i> ' ); 
}
else { 
	$last = new WP_Query('posts_per_page=1&order=ASC');
	$last->the_post();

	echo '<a href="' . get_permalink() . '">Следующий проект →</a>';
